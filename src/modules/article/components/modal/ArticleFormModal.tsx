...
export const ArticleFormModal = ({ isOpen, onClose, article, onSuccess }: ArticleFormModalProps) => {
  if (!isOpen) return null
  return article ? <EditForm article={article} onClose={onClose} onSuccess={onSuccess} /> : <CreateForm onClose={onClose} onSuccess={onSuccess} />
}
...
const CreateForm = ({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) => {
  ...
  return (
    <>
      ...
      <ConfirmModal isOpen={showConfirm} onClose={() => setShowConfirm(false)} onConfirm={() => { setShowConfirm(false); method.handleSubmit((data) => onSubmit(data as never))() }} title="Create Article" message="Are you sure you want to create this article?" confirmText="Create" variant="info" isLoading={isLoading} />
    </>
  )
}

const EditForm = ({ article, onClose, onSuccess }: { article: Article; onClose: () => void; onSuccess: () => void }) => {
      ...
      <ModalWrapper title="Edit Article" onClose={onClose} isLoading={isLoading}>
        <FormProvider {...method}>
          <form onSubmit={(e) => e.preventDefault()} className="p-6 space-y-4">
            ...
            <ArticleFormFields method={method} isLoading={isLoading} isEdit article={article} />
            <FormActions onClose={onClose} isLoading={isLoading} onClick={async () => (await method.trigger()) && setShowConfirm(true)} label="Update" />
          </form>
        </FormProvider>
      </ModalWrapper>
      <ConfirmModal isOpen={showConfirm} onClose={() => setShowConfirm(false)} onConfirm={() => { setShowConfirm(false); method.handleSubmit((data) => onSubmit(data as never))() }} title="Update Article" message="Are you sure you want to update this article?" confirmText="Update" variant="info" isLoading={isLoading} />
    </>
  )
}
...
const ArticleFormFields = ({ method, isLoading, isEdit, article }: { method: UseFormReturn<any>; isLoading: boolean; isEdit: boolean; article?: Article | null }) => {
  ...
  return (
    <>
      ...
      <FormSelect name="status" label="Status" options={ARTICLE_STATUS_OPTIONS} placeholder="Select Status" required disabled={isLoading} />
      ...
      <ImageUploader label="Image" value={method.watch('picture') || (isEdit && article ? getPictureUrl(article.picture) : '')} onChange={(file, _preview, uploadedUrl) => { if (uploadedUrl) method.setValue('picture', uploadedUrl, { shouldValidate: !isEdit }); else if (!file) method.setValue('picture', '') }} onUpload={uploadFileToS3} required={!isEdit} disabled={isLoading} error={method.formState.errors.picture?.message as string} />
      <FormTextArea name="content" label="Content" placeholder="Write article content here..." rows={8} required disabled={isLoading} />
    </>
  )
}
